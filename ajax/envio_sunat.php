...
switch($opc){
 ...
 function envio_sunat($idventa){
        ...
        //nota de credito
        $venta['relacionado_serie'] = $datos_comprobante['codigo_documento']; 
        $venta['relacionado_numero'] = $datos_comprobante['codigo_documento']; 
        $venta['relacionado_motivo_codigo'] = $datos_comprobante['codigo_documento']; 
        $venta['relacionado_tipo_documento'] = $datos_comprobante['codigo_documento']; 
...
$pdf->Ln(4);
 ...
 $totalVenta = explode(".",  $venta['total_venta']);
      ...
      $letras='SON'.': '.strtoupper(NumeroALetras::convertir($total ,  $totalVenta[1].'/100 '.$venta['moneda'], mb_convert_encoding("céntimos",'ISO-8859-1', 'UTF-8')));
        ...
        $pdf->Image($rutaqr, 25,$total_y - $tamano_y - 65,$tamano_x,$tamano_y);
         $pdf->Ln(50);
$pdf->SetFont('Arial', '', 7);
        ...
        $pdf->Cell(120, 1, $empresa["link_sistema"], "0","C");
...
function imprimir_pdf($empresa,$sede,$venta, $detalle,$variables_diversas_model){
       $pdf = new fpdf();
        ...
        $pdf->SetFont('Arial','',9);
        $pdf->Ln(4);
        $pdf->Cell(100,6,$empresa["telefono_fijo"]."-".$empresa["telefono_movil"],"0","C");
        $pdf->Ln(5);
        $pdf->SetFont('Arial','B',9);
    ...
    $pdf->SetFont('Arial','',9);
    $pdf->Ln(4);
    $pdf->Cell(100,6,"MONEDA"." : ".$venta["moneda"],'L,R',"0","L");
    $pdf->SetFont('Arial','',9);
    $pdf->Ln(4);
    $pdf->Cell(100,6,"MODO DE PAGO"." : ".$venta["forma_pago"],'L,B,R',"0","L");
...
$pdf->Ln(1);
 ...
 $pdf->Cell(180, 2, '_______________________________________________________________________________________________________________________', 0, 1, 'L');
     ...
     $pdf->Ln(1);
      ...
      $letras='IMPORTE EN LETRAS'.': '.strtoupper(NumeroALetras::convertir($total ,  $totalVenta[1].'/100 '.$venta['moneda'], mb_convert_encoding("céntimos",'ISO-8859-1', 'UTF-8')));
      //  $numLetras=$obj_venta->total_letras($total,'SOLES');
       // $letras='IMPORTE EN LETRAS'.': '.$numLetras['letter'];
     
        $pdf->MultiCell(100,5,$letras,"0","C");
    ...
    $pdf->SetFillColor( 0, 32, 96 );
    $pdf->SetDrawColor(0, 32, 96);
    $pdf->Cell(80, 6, $tipo_documento." DE VENTA ELECTRONICA", 1, 0, 'C', True);
    ...
    $pdf->SetXY(120,54);
    $pdf->Cell(80,6,utf8_decode("FECHA DE VENC").": ".$venta["fecha_emision"],"0","L");
   // detalle de la venta
